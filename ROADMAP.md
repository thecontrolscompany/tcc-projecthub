...
- [x] **Parts catalog default-part visibility** - Price Book "Default For" column + "Defaults only" filter on the Installation/Controls tabs, plus a new "Default Assignments" tab for reverse lookups (e.g. an AHU's default Return Air Sensor part, with part #/manufacturer/pricing). Derived from existing `def`/`defaultWhen` component flags via `src/modules/hvac-estimator/shared/defaultPartsIndex.js` - no schema changes.
...
- [x] **Site walkthrough videos** - customer portal Walkthroughs tab embedding Insta360 cloud 360° videos; hosting stays on Insta360 (their player can't be iframed cross-origin, so TCC-branded cards open the player in a new tab); admin pastes share links per project in the ops modal, backend scrapes title/duration/cover from the share page; customer view groups clips by week with one card per area/floor and a branded fallback tile if a cover token expires. Location-overlay follow-up is Planned. Spec: `codex/roadmap-walkthrough-location-overlay.md`
...
- [ ] **Walkthrough location overlay** (Priority: Low) — show where a 360° walkthrough is on the site plan. Phase 1: static "you are here" floor-plan marker per clip (keeps Insta360 hosting, days of work). Phase 2: dot moving along a path synced to playback (requires re-hosting video off Insta360 + a per-video click-along authoring step, multi-week). Spec: `codex/roadmap-walkthrough-location-overlay.md`
