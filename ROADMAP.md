...
- [x] **Site walkthrough videos** - customer portal Walkthroughs tab embedding Insta360 cloud 360° videos; hosting stays on Insta360 (their player can't be iframed cross-origin, so TCC-branded cards open the player in a new tab); admin pastes share links per project in the ops modal, backend scrapes title/duration/cover from the share page; customer view groups clips by week with one card per area/floor and a branded fallback tile if a cover token expires. Spec: `codex/roadmap-walkthrough-location-overlay.md`
- [x] **Walkthrough location overlay — hybrid PSV player** - opt-in self-hosted equirectangular MP4 walkthroughs in Photo Sphere Viewer with a live floor-plan position dot, click-along waypoint JSON, RLS-protected viewer data, admin creation flow, and distinct customer cards; existing Insta360 walkthroughs remain unchanged. Spec: `codex/roadmap-walkthrough-location-overlay.md`
