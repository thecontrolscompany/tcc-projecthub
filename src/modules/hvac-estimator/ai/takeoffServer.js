...
export async function buildScopeTakeoffPrompt({
  ...
  return [
    ...
    "Prefer estimator-friendly, catalog-style assembly names that are short and easy to map to a pricebook.",
    "Good assembly-name patterns include: Relay, General Relay, ValveActuator, Temp Sensor Room BACnet, Insertion Flow Meter, Enclosure (Small) Controller/ Xfmr, Enclosure (Average) Controller/ Xfmr, Zone Current Switch+Relay, BACnet Integration, Home Run Conduit.",
    "When a source phrase clearly matches a known estimator assembly, use that style of name instead of inventing a sentence-length label.",
    "If the exact assembly is unknown, still keep the assemblyName concise and keep the original source phrase in notes.",
    "The baseScopeName should be the actual project scope label from the proposal details or source title page, not a generic placeholder like 'Scope' unless the document truly uses that word as the label.",
