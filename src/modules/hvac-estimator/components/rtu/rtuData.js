...
export const RTU_COMPS = [
  // Temperature
  { id:"rtu-oa-t",    emtAID:"60102", plnAID:"60102", name:"OA Temp Sensor",        cat:"Temperature", controlsId: "CTL-AI", def:true,  defaultWhen: () => true, desc:"Outside air temp at intake" },
  { id:"rtu-ma-t",    emtAID:"60094", plnAID:"60094", name:"Mixed Air Temp",         cat:"Temperature", controlsId: "CTL-AI", def:true,  defaultWhen: () => true, desc:"Mixed air sensor after mixing section" },
  { id:"rtu-sa-t",    emtAID:"60073", plnAID:"60073", name:"Supply Air Temp",        cat:"Temperature", controlsId: "CTL-AI", def:true,  defaultWhen: () => true, desc:"Discharge air temp sensor" },
  // Cooling
  { id:"rtu-dx2",     groupId:"rtu-cooling-stage", emtAID:"60106", plnAID:"60106", name:"DX Cooling 2-Stage",     cat:"Controls", controlsId: "CTL-BO",    def:false, showWhen: cfg => normalizeRtuCfg(cfg).coolingType === "dx-2stage", defaultWhen: cfg => normalizeRtuCfg(cfg).coolingType === "dx-2stage", desc:"2-stage DX cooling relay outputs" },
  { id:"rtu-dx4",     groupId:"rtu-cooling-stage", emtAID:"60105", plnAID:"60105", name:"DX Cooling 4-Stage",     cat:"Controls", controlsId: "CTL-BO",    def:true,  showWhen: cfg => normalizeRtuCfg(cfg).coolingType === "dx-4stage", defaultWhen: cfg => normalizeRtuCfg(cfg).coolingType === "dx-4stage", desc:"4-stage DX cooling relay outputs" },
  ...
  // Heating
  { id:"rtu-htg2",    groupId:"rtu-heating-type", emtAID:"60109", plnAID:"60109", name:"Electric Heat 2-Stage",  cat:"Actuators", controlsId: "CTL-BO",   def:false, showWhen: cfg => normalizeRtuCfg(cfg).heatingType === "electric-2stage", defaultWhen: cfg => normalizeRtuCfg(cfg).heatingType === "electric-2stage", desc:"2-stage electric heat relay" },
  { id:"rtu-htg3",    groupId:"rtu-heating-type", emtAID:"60107", plnAID:"60107", name:"Electric Heat 3-Stage",  cat:"Actuators", controlsId: "CTL-BO",   def:true,  showWhen: cfg => normalizeRtuCfg(cfg).heatingType === "electric-3stage", defaultWhen: cfg => normalizeRtuCfg(cfg).heatingType === "electric-3stage", desc:"3-stage electric heat relay" },
  { id:"rtu-hw-vlv",  groupId:"rtu-heating-type", emtAID:"60077", plnAID:"60077", name:"HW Heating Valve",       cat:"Actuators", controlsId: "CTL-AO",   def:false, showWhen: cfg => normalizeRtuCfg(cfg).heatingType === "hw-valve", defaultWhen: cfg => normalizeRtuCfg(cfg).heatingType === "hw-valve", desc:"Hot water heating valve actuator" },
  // Dampers / Economizer
  { id:"rtu-oa-d",    emtAID:"60069", plnAID:"60069", name:"OA Damper Actuator",     cat:"Actuators", controlsId: "CTL-BO",   def:true,  defaultWhen: () => true, desc:"Economizer OA damper actuator" },
  { id:"rtu-filter",  emtAID:"60059", plnAID:"60059", name:"Filter DP Switch",       cat:"Pressure", controlsId: "CTL-AI",    def:true,  defaultWhen: () => true, desc:"Differential pressure across filter" },
  // Fan / Drive
  { id:"rtu-vfd-sf",  emtAID:"60129", plnAID:"60129", name:"Supply Fan VFD",         cat:"Drives", controlsId: "CTL-AO",      def:true,  defaultWhen: () => true, desc:"VFD on supply fan" },
  { id:"rtu-vfd-rf",  emtAID:"60129", plnAID:"60129", name:"Return Fan VFD",         cat:"Drives", controlsId: "CTL-AO",      def:false, showWhen: cfg => normalizeRtuCfg(cfg).returnFan === true, desc:"VFD on return fan" },
  { id:"rtu-sa-sp",   emtAID:"60100", plnAID:"60100", name:"SA Static Pressure",     cat:"Pressure", controlsId: "CTL-AI",    def:true,  defaultWhen: () => true, desc:"Supply duct static pressure" },
  // Safety
  { id:"rtu-freeze",  emtAID:"60056", plnAID:"60056", name:"Freezestat",             cat:"Safety", controlsId: "CTL-BI",      def:true,  defaultWhen: () => true, desc:"Low-limit freeze protection" },
  { id:"rtu-smoke",   emtAID:"60122", plnAID:"60122", name:"Smoke Detector",         cat:"Safety", controlsId: "CTL-BI",      def:true,  defaultWhen: () => true, desc:"Duct smoke detector" },
  // Controls
